...
const AvailablePropertiesPage: React.FC = () => {
  ...
  const filteredAndSortedProperties = useMemo(() => {
    ...
    if (filterStatus !== 'All') {
      ...
      // Default to showing Live and Fully Funded on the "All" filter for this page
      filtered = filtered.filter(p => p.status === PropertyStatus.Live || p.status === PropertyStatus.FullyFunded);
    }
    ...
    return [...filtered].sort((a, b) => {
      ...
      // Add ROI sort if metrics are readily available later
      return 0;
    });
    ...
    <div className={`bg-neutral-50 min-h-screen`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <PageTitle 
            ...
            subtitle="Discover your next venture in luxury fractional ownership. Each property is carefully selected for its potential and appeal." 
        />
        ...
        <div className={`mb-8 p-5 bg-white shadow-lg rounded-xl flex flex-col md:flex-row gap-4 items-center border border-neutral-200`}>
          ...
          >
            <option value="All">All (Live & Funded)</option>
            <option value={PropertyStatus.Live}>Live</option>
            <option value={PropertyStatus.FullyFunded}>Fully Funded</option>
            {/* Removed Draft and Closed from user-facing property list filters */}
          </select>
           <select 
            value={sortBy} 
            onChange={(e) => setSortBy(e.target.value as 'name' | 'investmentGoal' | 'roi')}
            className="p-3 border border-neutral-300 rounded-md shadow-sm focus:ring-2 focus:ring-primary/50 focus:border-primary sm:text-sm bg-white transition-shadow w-full md:w-auto"
          >
        ...
        {filteredAndSortedProperties.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-xl text-neutral-500">No properties match your current filters.</p>
            <p className="text-sm text-neutral-400 mt-2">Try adjusting your search or filter criteria.</p>
          </div>
        )}
